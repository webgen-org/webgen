...
namespace {/**
 * Nette Framework (version 2.0-dev released on 2011-05-28, http://nette.org)
 *
...
createService(IContainer$container);}}namespace Nette\Forms{use
Nette;interface
IControl{function
...
getAuthorizationHandler();}}namespace Nette\Latte{use
Nette;interface
IMacro{function
initialize();function
finalize();function
...
run(){$httpRequest=$this->context->httpRequest;$httpResponse=$this->context->httpResponse;if($this->allowedMethods){$method=$httpRequest->getMethod();if(!in_array($method,$this->allowedMethods,TRUE)){$httpResponse->setCode(Nette\Http\IResponse::S501_NOT_IMPLEMENTED);$httpResponse->setHeader('Allow',implode(',',$this->allowedMethods));echo'<h1>Method '.htmlSpecialChars($method).' is not implemented</h1>';return;}}$request=NULL;$repeatedError=FALSE;do{try{if(count($this->requests)>self::$maxLoop){throw
new
...
storeRequest($expiration='+ 10 minutes'){$session=$this->context->session->getNamespace('Nette.Application/requests');do{$key=Nette\Utils\Strings::random(5);}while(isset($session[$key]));$session[$key]=end($this->requests);$session->setExpiration($expiration,$key);return$key;}function
restoreRequest($key){$session=$this->context->session->getNamespace('Nette.Application/requests');if(isset($session[$key])){$request=clone$session[$key];unset($session[$key]);$request->setFlag(Request::RESTORED,TRUE);$this->presenter->sendResponse(new
...
__construct($baseDir,Nette\DI\IContainer$context){$this->baseDir=$baseDir;$this->context=$context;}function
...
instanceof
Nette\Callback){$metadata=array(self::PRESENTER_KEY=>'Nette:Simple','callback'=>$metadata);}$this->flags=$flags|self::$defaultFlags;$this->setMask($mask,$metadata);}function
...
new
Application\Request($presenter,$httpRequest->getMethod(),$params,$httpRequest->getPost(),$httpRequest->getFiles(),array(Application\Request::SECURED=>$httpRequest->isSecured()));}function
constructUrl(Application\Request$appRequest,Nette\Http\Url$refUrl){$params=$appRequest->getParams();$presenter=$appRequest->getPresenterName();if(strncasecmp($presenter,$this->module,strlen($this->module))===0){$params[self::PRESENTER_KEY]=substr($presenter,strlen($this->module));}else{return
...
callback($params['callback'])->invokeNamedArgs($params);}}function
...
setContext(Nette\DI\IContainer$context){$this->context=$context;return$this;}final
function
getContext(){return$this->context;}}}namespace Nette\Application\UI{use
Nette;class
BadSignalException
extends
...
Nette\Templating\FileTemplate;$presenter=$this->getPresenter(FALSE);$template->onPrepareFilters[]=callback($this,'templatePrepareFilters');$template->control=$this;$template->presenter=$presenter;if($presenter
instanceof
...
addProtection($message=NULL,$timeout=NULL){$session=$this->getSession()->getNamespace('Nette.Forms.Form/CSRF');$key="key$timeout";if(isset($session->$key)){$token=$session->$key;}else{$session->$key=$token=Nette\Utils\Strings::random();}$session->setExpiration($timeout,$key);$this[self::PROTECTOR_ID]=new
...
Nette\Utils\Arrays::mergeTree($request->getPost(),$request->getFiles());}else{return$request->getParams();}}function
...
getFlashSession(){if(empty($this->params[self::FLASH_KEY])){$this->params[self::FLASH_KEY]=Nette\Utils\Strings::random(4);}return$this->getSession('Nette.Application.Flash/'.$this->params[self::FLASH_KEY]);}function
setContext(Nette\DI\IContainer$context){$this->context=$context;return$this;}final
function
...
getUser(){return$this->context->user;}}}namespace Nette\Reflection{use
Nette;use
...
new
Nette\InvalidStateException('Output cache has already been saved.');}$this->cache->save($this->key,ob_get_flush(),$this->dependencies);$this->cache=NULL;}}}namespace Nette\Caching\Storages{use
Nette;class
DevNullStorage
extends
Nette\Object
...
new
Nette\DirectoryNotFoundException("Directory '$dir' not found.");}$this->useDirs=(bool)self::$useDirectories;$this->journal=$journal;if(mt_rand()/mt_getrandmax()<self::$gcProbability){$this->clean(array());}}function
...
__construct($containerClass='Nette\DI\Container'){self::$instance=$this;$this->container=new$containerClass;foreach(get_class_methods($this)as$name){if(substr($name,0,13)==='createService'){$this->container->addService(strtolower($name[13]).substr($name,14),array(get_called_class(),$name));}}$this->container->params=new
...
new
Nette\NotSupportedException('Required function ini_set() is disabled.');}}private
static
function
...
new
Nette\Application\PresenterFactory($container->params['appDir'],$container);}static
function
createServiceRouter(DI\Container$container){return
new
Nette\Application\Routers\RouteList;}static
function
...
new
Nette\Http\Response;}static
function
createServiceHttpContext(DI\Container$container){return
new
Nette\Http\Context($container->httpRequest,$container->httpResponse);}static
function
createServiceSession(DI\Container$container){return
new
Nette\Http\Session($container->httpRequest,$container->httpResponse);}static
function
...
new
Nette\Caching\Storages\PhpFileStorage($dir);}static
function
createServiceCacheJournal(DI\Container$container){return
new
Nette\Caching\Storages\FileJournal($container->params['tempDir']);}static
function
...
new
Nette\InvalidStateException("Unable to write to directory '$dir'. Make this directory writable.");}$useDirs=@file_put_contents("$dir/$uniq/_",'')!==FALSE;@unlink("$dir/$uniq/_");@rmdir("$dir/$uniq");$code.=self::generateCode('Nette\Caching\Storages\FileStorage::$useDirectories = ?',$useDirs);return$code;}}class
DateTime
...
class
Framework{const
NAME='Nette Framework',VERSION='2.0-beta',REVISION='0f302dd released on 2011-05-28';public
...
Nette;class
Config{private
static$extensions=array('ini'=>'Nette\Config\IniAdapter','neon'=>'Nette\Config\NeonAdapter');final
function
__construct(){throw
new
Nette\StaticClassException;}static
...
new
Nette\InvalidStateException("There is not section [$section] in file '$file'.");}$data=$data[$section];}return$data;}static
function
...
new
Nette\InvalidStateException("Invalid key '$key' in section [$secName] in file '$file'.");}}$cursor=$val;}$secData=$tmp;}$parts=$separator?explode($separator,strtr($secName,':',$separator)):array($secName);if(count($parts)>1){$parent=trim($parts[1]);if(!isset($data[$parent])||!is_array($data[$parent])){throw
new
Nette\InvalidStateException("Missing parent section [$parent] in file '$file'.");}$secData=Nette\Utils\Arrays::mergeTree($secData,$data[$parent]);$secName=trim($parts[0]);if($secName===''){throw
new
Nette\InvalidStateException("Invalid empty section name in file '$file'.");}}}if(self::$keySeparator){$cursor=&$data;foreach(explode(self::$keySeparator,$secName)as$part){if(!isset($cursor[$part])||is_array($cursor[$part])){$cursor=&$cursor[$part];}else{throw
new
Nette\InvalidStateException("Invalid section [$secName] in file '$file'.");}}}else{$cursor=&$data[$secName];}if(is_array($secData)&&is_array($cursor)){$secData=Nette\Utils\Arrays::mergeTree($secData,$cursor);}$cursor=$secData;}return$data;}static
function
...
new
Nette\InvalidStateException("Invalid section '$secName'.");}$output[]="[$secName]";self::build($secData,$output,'');$output[]='';}if(!file_put_contents($file,implode(PHP_EOL,$output))){throw
new
...
Nette\InvalidStateException("Missing parent section '$parent' in file '$file'.");}$secData=Nette\Utils\Arrays::mergeTree($secData,$data[$parent]);$secName=trim($parts[0]);if($secName===''){throw
new
Nette\InvalidStateException("Invalid empty section name in file '$file'.");}}}$data[$secName]=$secData;}return$data;}static
function
...
static;$connection->onQuery[]=callback($panel,'logQuery');Debugger::$bar->addPanel($panel);Debugger::$blueScreen->addPanel(callback($panel,'renderException'),__CLASS__);}}function
...
Nette\Object
implements
Nette\Database\ISupplementalDriver{public$supports=array('meta'=>TRUE);private$connection;function
...
Nette\Object
implements
Nette\Database\ISupplementalDriver{public$supports=array('meta'=>TRUE);private$connection;private$fmtDateTime;function
...
Nette\Object
implements
Nette\Database\ISupplementalDriver{public$supports=array('meta'=>FALSE);private$connection;private$fmtDateTime;function
__construct(Nette\Database\Connection$connection,array$options){$this->connection=$connection;$this->fmtDateTime=isset($options['formatDateTime'])?$options['formatDateTime']:'U';}function
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$cmd=strtoupper(substr(ltrim($sql),0,6));$this->arrayMode=$cmd==='INSERT'||$cmd==='REPLAC'?'values':'assoc';$sql=Nette\Utils\Strings::replace($sql,'~\'.*?\'|".*?"|:[a-zA-Z0-9_]+:|\?~s',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
new
Nette\InvalidArgumentException("Service name must be a non-empty string, ".gettype($name)." given.");}if(isset($this->registry[$name])||method_exists($this,"createService$name")){throw
new
Nette\InvalidStateException("Service '$name' has already been registered.");}if(is_string($tags)){$tags=array(self::TAG_TYPEHINT=>array($tags));}elseif(is_array($tags)){foreach($tags
...
instanceof
Nette\Callback){$this->registry[$name]=$service;$this->tags[$name]=$tags;return$this;}else{$factory=$service;}$this->factories[$name]=array(callback($factory));$this->tags[$name]=$tags;$this->registry[$name]=&$this->factories[$name][1];return$service;}function
...
new
Nette\InvalidStateException("Circular reference detected for services: ".implode(', ',array_keys($this->creating)).".");}if(isset($this->factories[$name])){list($factory)=$this->factories[$name];if(!$factory->isCallable()){throw
new
Nette\InvalidStateException("Unable to create service '$name', factory '$factory' is not callable.");}$this->creating[$name]=TRUE;try{$service=$factory($this);}catch(\Exception$e){}}elseif(method_exists($this,$factory='createService'.ucfirst($name))){$this->creating[$name]=TRUE;try{$service=$this->$factory();}catch(\Exception$e){}}else{throw
new
...
new
Nette\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not object.");}elseif(isset($this->tags[$name][self::TAG_TYPEHINT][0])&&!$service
instanceof$this->tags[$name][self::TAG_TYPEHINT][0]){throw
new
Nette\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not '{$this->tags[$name][self::TAG_TYPEHINT][0]}' type.");}unset($this->factories[$name]);return$this->registry[$name]=$service;}function
...
new
Nette\InvalidArgumentException("Missing parameter '$param'.");}elseif(!is_scalar($val=$that->params[$param])){throw
new
Nette\InvalidStateException("Parameter '$param' is not scalar.");}return$val;},$s);}return$s;}function&__get($name){if(!isset($this->registry[$name])){$this->getService($name);}return$this->registry[$name];}function
...
new
Nette\InvalidStateException("The definition of service '$name' is missing factory method.");}if(isset($definition['tags'])){$tags=(array)$definition['tags'];array_walk_recursive($tags,$expander);}else{$tags=NULL;}$container->addService($name,$factory,$tags);}}function
...
new
Nette\InvalidStateException("The definition of service '$name' is missing factory method.");}$tags=isset($definition['tags'])?$this->argsExport(array($definition['tags'])):'NULL';$code.="\$container->addService($name, $factory, $tags);\n\n";}return$code;}private
function
...
createService(Nette\DI\IContainer$container){if(!class_exists($this->class)){throw
new
Nette\InvalidStateException("Cannot instantiate service, class '$this->class' not found.");}return
new$this->class;}}}namespace Nette\Diagnostics{use
Nette;class
Bar
extends
...
function
match($subject,$pattern,$flags=0,$offset=0){Nette\Diagnostics\Debugger::tryError();$res=preg_match($pattern,$subject,$m,$flags,$offset);self::catchPregError($pattern);if($res){return$m;}}static
function
matchAll($subject,$pattern,$flags=0,$offset=0){Nette\Diagnostics\Debugger::tryError();$res=preg_match_all($pattern,$subject,$m,($flags&PREG_PATTERN_ORDER)?$flags:($flags|PREG_SET_ORDER),$offset);self::catchPregError($pattern);return$m;}static
function
...
RegexpException
extends\Exception{}}namespace Nette\Diagnostics{use
Nette;class
Logger
extends
Nette\Object{const
...
<?php }return
ob_get_clean();}}}namespace Nette\Forms{use
Nette;class
ControlGroup
extends
...
loadHttpData(){$path=explode('[',strtr(str_replace(array('[]',']'),'',$this->getHtmlName()),'.','_'));$this->setValue(Nette\Utils\Arrays::get($this->getForm()->getHttpData(),$path));}function
...
sanitize($value){if($this->control->maxlength&&Nette\Utils\Strings::length($value)>$this->control->maxlength){$value=iconv_substr($value,0,$this->control->maxlength,'UTF-8');}return
...
Nette\Utils\Arrays::get($this->files,$args);}final
function
...
new
Nette\InvalidStateException('session_start(): '.$e->getMessage(),0,$e);}self::$started=TRUE;if($this->regenerationNeeded){session_regenerate_id(TRUE);$this->regenerationNeeded=FALSE;}unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];if(empty($nf)){$nf=array('C'=>0);}else{$nf['C']++;}$browserKey=$this->request->getCookie('nette-browser');if(!$browserKey){$browserKey=Nette\Utils\Strings::random();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$namespace=>$metadata){if(is_array($metadata)){foreach($metadata
...
getNamespace($name,$class='Nette\Http\SessionNamespace'){return
...
NULL;}$this->session=$session=$this->context->session->getNamespace('Nette.Web.User/'.$this->namespace);if(!$session->identity
instanceof
...
function
getAuthorizationHandler(){return$this->context->authorizator;}}}namespace Nette\Iterators{use
Nette;class
CachingIterator
...
Engine
extends
Nette\Object{public$parser;function
...
__invoke($s){$this->parser->context=Parser::CONTEXT_TEXT;$this->parser->escape='Nette\Templating\DefaultHelpers::escapeHtml|';$this->parser->setDelimiters('\\{(?![\\s\'"{}*])','\\}');return$this->parser->parse($s);}}class
HtmlNode
extends
Nette\Object{public$name;public$isEmpty=FALSE;public$attrs=array();public$closing=FALSE;public$offset;function
__construct($name){$this->name=$name;$this->isEmpty=isset(Nette\Utils\Html::$emptyElements[strtolower($this->name)]);}}class
MacroNode
extends
Nette\Object{public$macro;public$name;public$isEmpty=FALSE;public$args;public$modifiers;public$closing=FALSE;public$tokenizer;public$offset;public$parentNode;public$content;public$data;function
...
function
createCache(Nette\Caching\IStorage$cacheStorage,$key,&$parents,$args=NULL){if($args){if(array_key_exists('if',$args)&&!$args['if']){return$parents[]=(object)NULL;}$key=array_merge(array($key),array_intersect_key($args,range(0,count($args))));}if($parents){end($parents)->dependencies[Nette\Caching\Cache::ITEMS][]=$key;}$cache=new
Nette\Caching\Cache($cacheStorage,'Nette.Templating.Cache');if($helper=$cache->start($key)){$helper->dependencies=array(Nette\Caching\Cache::TAGS=>isset($args['tags'])?$args['tags']:NULL,Nette\Caching\Cache::EXPIRATION=>isset($args['expire'])?$args['expire']:'+ 7 days');$parents[]=$helper;}return$helper;}}use
...
function
compile(MacroNode$node,$def){$writer=Latte\PhpWriter::using($node,$this->parser->escape);if(is_string($def)){$code=$writer->write($def);}else{$code=callback($def)->invoke($node,$writer);if($code===FALSE){return
...
finalize(){return
array('list($_l, $_g) = Nette\Latte\Macros\CoreMacros::initRuntime($template, '.var_export($this->parser->templateId,TRUE).')');}function
...
macroInclude(MacroNode$node,$writer){$code=$writer->write('Nette\Latte\Macros\CoreMacros::includeTemplate(%node.word, %node.array? + $template->getParams(), $_l->templates[%var])',$this->parser->templateId);if($node->modifiers){return$writer->write('echo %modify',$code.'->__toString(TRUE)');}else{return$code.'->render()';}}function
...
macroDump(MacroNode$node,$writer){$args=$writer->formatArgs();return$writer->write('Nette\Diagnostics\Debugger::barDump('.($node->args?"array(%var => $args)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))',$args);}function
...
macroIncludeBlock(MacroNode$node,$writer){return$writer->write('Nette\Latte\Macros\CoreMacros::includeTemplate(%node.word, %node.array? + get_defined_vars(), $_l->templates[%var])->render()',$this->parser->templateId);}function
...
macroIfCurrent(MacroNode$node,$writer){return$writer->write(($node->args?'try { $presenter->link(%node.word, %node.array?); } catch (Nette\Application\UI\InvalidLinkException $e) {}':'').'; if ($presenter->getLastCreatedRequestFlag("current")):');}function
macroContentType(MacroNode$node,$writer){if(strpos($node->args,'html')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeHtml|';$this->parser->context=Latte\Parser::CONTEXT_TEXT;}elseif(strpos($node->args,'xml')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeXml';$this->parser->context=Latte\Parser::CONTEXT_NONE;}elseif(strpos($node->args,'javascript')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeJs';$this->parser->context=Latte\Parser::CONTEXT_NONE;}elseif(strpos($node->args,'css')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeCss';$this->parser->context=Latte\Parser::CONTEXT_NONE;}elseif(strpos($node->args,'plain')!==FALSE){$this->parser->escape='';$this->parser->context=Latte\Parser::CONTEXT_NONE;}else{$this->parser->escape='$template->escape';$this->parser->context=Latte\Parser::CONTEXT_NONE;}if(strpos($node->args,'/')){return$writer->write('$netteHttpResponse->setHeader("Content-Type", %node.word)');}}static
function
...
instanceof
Nette\Application\UI\Control){foreach($control->getComponents(FALSE,'Nette\Application\UI\Control')as$child){if($child->isControlInvalid()){$child->render();}}}}}}namespace Nette\Utils{use
Nette;use
Nette\Utils\Strings;class
Tokenizer
extends
...
contextText(){$matches=$this->match('~
		...
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['htmlcomment'])){$this->context=self::CONTEXT_COMMENT;$this->escape='Nette\Templating\DefaultHelpers::escapeHtmlComment';}elseif(empty($matches['closing'])){$this->htmlNodes[]=$node=new
...
contextCData(){$node=end($this->htmlNodes);$matches=$this->match('~
			...
			'.$this->macroRe.'           ##  curly tag
		~xsi');if($matches&&empty($matches['macro'])&&empty($matches['comment'])){$node->closing=TRUE;$node->offset=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='Nette\Templating\DefaultHelpers::escapeHtml';}return$matches;}private
function
contextTag(){$matches=$this->match('~
			(?P<end>\ ?/?>)(?P<tagnewline>[ \t]*\n)?|  ##  end of HTML tag
			'.$this->macroRe.'|          ##  curly tag
			\s*(?P<attr>[^\s/>={]+)(?:\s*=\s*(?P<value>["\']|[^\s/>{]+))? ## begin of HTML attribute
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['end'])){$node=end($this->htmlNodes);$isEmpty=!$node->closing&&(strpos($matches['end'],'/')!==FALSE||$node->isEmpty);if($isEmpty){$matches[0]=(Nette\Utils\Html::$xhtml?' />':'>').(isset($matches['tagnewline'])?$matches['tagnewline']:'');}if(!empty($node->attrs)){$code=substr($this->output,$node->offset).$matches[0];$this->output=substr($this->output,0,$node->offset);$this->writeAttrsMacro($code,$node->attrs,$node->closing);if($isEmpty){$this->writeAttrsMacro('',$node->attrs,TRUE);}$matches[0]='';}if($isEmpty){$node->closing=TRUE;}if(!$node->closing&&(strcasecmp($node->name,'script')===0||strcasecmp($node->name,'style')===0)){$this->context=self::CONTEXT_CDATA;$this->escape='Nette\Templating\DefaultHelpers::escape'.(strcasecmp($node->name,'style')?'Js':'Css');}else{$this->context=self::CONTEXT_TEXT;$this->escape='Nette\Templating\DefaultHelpers::escapeHtml|';if($node->closing){array_pop($this->htmlNodes);}}}else{$name=$matches['attr'];$value=isset($matches['value'])?$matches['value']:'';$node=end($this->htmlNodes);if(Strings::startsWith($name,self::N_PREFIX)){$name=substr($name,strlen(self::N_PREFIX));if($value==='"'||$value==="'"){if($matches=$this->match('~(.*?)'.$value.'~xsi')){$value=$matches[1];}}$node->attrs[$name]=$value;$matches[0]='';}elseif($value==='"'||$value==="'"){$this->context=self::CONTEXT_ATTRIBUTE;$this->quote=$value;$this->escape=strncasecmp($name,'on',2)?('Nette\Templating\DefaultHelpers::escapeHtml'.(strcasecmp($name,'style')?"|$value":'Css')):"Nette\\Templating\\DefaultHelpers::escapeHtmlJs|$value";}}return$matches;}private
function
...
PhpWriter
extends
Nette\Object{private$argsTokenizer;private$modifiers;private$escape;static
function
...
Nette\Utils\Strings::replace($mask,'#([,+]\s*)?%(node\.word|node\.array|node\.args|modify|var)(\?)?(\s*\+\s*)?()#',function($m)use($me,$word,&$args){list(,$l,$macro,$cond,$r)=$m;switch($macro){case'node.word':$code=$me->formatWord($word);break;case'node.args':$code=$me->formatArgs();break;case'node.array':$code=$me->formatArray();$code=$cond&&$code==='array()'?'':$code;break;case'modify':$code=$me->formatModifiers(array_shift($args));break;case'var':$code=var_export(array_shift($args),TRUE);break;}if($cond&&$code===''){return$r?$l:$r;}else{return$l.$code.$r;}});}function
...
new
Nette\InvalidArgumentException("Name must not contain line separator.");}$tmp[$email]=$name;}}else{$value=(string)$value;if(!Strings::checkEncoding($value)){throw
new
...
getClassName(){return($tmp=Nette\Utils\Strings::match($this,'#>\s+([a-z0-9_\\\\]+)#i'))?$tmp[1]:NULL;}function
...
Nette\StaticClassException;}static
function
loader($helper){$callback=callback('Nette\Templating\DefaultHelpers',$helper);if($callback->isCallable()){return$callback;}$callback=callback('Nette\Utils\Strings',$helper);if($callback->isCallable()){return$callback;}}static
function
...
instanceof
Caching\Storages\PhpFileStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$content=$cache->load($this->file);if($content===NULL){try{$content=$this->compile(file_get_contents($this->file));$content="<?php\n\n// source file: $this->file\n\n?>$content";}catch(FilterException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$content,array(Caching\Cache::FILES=>$this->file,Caching\Cache::CONSTS=>'Nette\Framework::REVISION'));$cache->release();$cached=$cache->load($this->file);}if($cached!==NULL&&$storage
instanceof
Caching\Storages\PhpFileStorage){Nette\Utils\LimitedScope::load($cached['file'],$this->getParams());flock($cached['handle'],LOCK_UN);fclose($cached['handle']);}else{Nette\Utils\LimitedScope::evaluate($content,$this->getParams());}}function
...
new
Caching\Storages\DevNullStorage;}return$this->cacheStorage;}}}namespace Nette\Utils{use
Nette;final
class
Arrays{final
function
...
function
grep(array$arr,$pattern,$flags=0){Nette\Diagnostics\Debugger::tryError();$res=preg_grep($pattern,$arr,$flags);Strings::catchPregError($pattern);return$res;}}final
class
...
getLength(){return$this->itemCount===NULL?$this->itemsPerPage:min($this->itemsPerPage,$this->itemCount-$this->getPageIndex()*$this->itemsPerPage);}}}namespace Nette\Diagnostics{use
Nette;final
class
Debugger{public
